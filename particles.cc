...
// set camera at a random position
GLfloat horizontal = -82.f;
GLfloat horizontal_view = 2.f;
GLfloat vertical = -82.f;

GLfloat scrollx = 0.f;
GLfloat scrolly = 0.f;
int scrollx_multiplier = 1;

double cursorx = 0;
double cursory = 0;

// TODO understand relationship a little bit better 
int horizontal_multiplier = 2;
int vertical_multiplier = 2;
...
#define FOUNTAIN_SWEEP_STEPS 28
...
static void draw_wall(void) {
    ...
    if (!wall_list)
    {
        ...
        glNewList(wall_list, GL_COMPILE_AND_EXECUTE);

        // glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE, red);

        glBegin(GL_QUAD_STRIP);
        glNormal3f(4.f, 0.f, 0.f);
        glVertex3f(4.f, 0.f, 0.f);
        glNormal3f(4.f, 0.f, 5.f);
        glVertex3f(4.f, 0.f, 5.f);
        glNormal3f(82.f, 0.f, 0.f);
        glVertex3f(82.f, 0.f, 0.f);
        glNormal3f(82.f, 0.f, 5.f);
        glVertex3f(82.f, 0.f, 5.f);
        glEnd();
...
static void draw_scene(GLFWwindow* window, double t)
{
    ...
    if (0 == victory) {
      ...
      mat4x4 view;
      vec3 eye = { horizontal, vertical, 0.7f };
      vec3 center = { horizontal + horizontal_multiplier + (scrollx * scrollx_multiplier), vertical + vertical_multiplier, 0.7f - scrolly};
      vec3 up = { 0.f, 0.f, 1.f };
...
bool collision(const GLfloat x1, const GLfloat y1, GLfloat & x2, GLfloat & y2, const GLfloat x3, const GLfloat y3, const GLfloat x4, const GLfloat y4) {
  bool result = false;
  result = ! (std::max(x1, x2) < std::min(x3, x4) || std::min(x1, x2) > std::max(x3, x4));
  result &= ! (std::max(y1, y2) < std::min(y3, y4) || std::min(y1, y2) > std::max(y3, y4));
  if (result) {
    if (x3 == x4 && x2 !=  x3)
      x2 = x3 + ((x2 > x3 ? -1 : 1) * 1.f);
    else if (y3 == y4 && y2 != y3)
...
static void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if (action == GLFW_PRESS) {
        switch (key) {
            ...
            case GLFW_KEY_LEFT:
            {
                if (-4 >= horizontal_multiplier)
                  horizontal_step = -1;
                else if (4 <= horizontal_multiplier)
                  horizontal_step = 1;
                if (-4 >= vertical_multiplier)
                  vertical_step = -1;
                else if (4 <= vertical_multiplier)
                  vertical_step = 1;
                horizontal_multiplier -= horizontal_step;
                vertical_multiplier -= vertical_step;
            ...
            case GLFW_KEY_UP:
            {
                GLfloat new_horizontal = horizontal + horizontal_multiplier;
                GLfloat new_vertical = vertical + vertical_multiplier;
                // printf("(%f, %f), (%f, %f) <=> (4.f, 0.f), (82.f, 0.f) -> %s\n", horizontal, vertical, new_horizontal, new_vertical, collision(horizontal, vertical, new_horizontal, new_vertical, 4.f, 0.f, 82.f, 0.f) ? "true" : "false")
                collision(horizontal, vertical, new_horizontal, new_vertical, 4.f, 0.f, 82.f, 0.f);
        ...
        if (-4 < horizontal && 4 > horizontal && -4 < vertical && 4 > vertical) {
          if (0 == victory)
            printf("Victory!\n");
          victory = 1;
        }
...
void cursor_position_callback(GLFWwindow* window, double x, double y)
{
    if (glfwGetInputMode(window, GLFW_CURSOR) == GLFW_CURSOR_DISABLED) {
        scrollx = (float) (x - cursorx) / 20.f;
        scrolly = (float) (y - cursory) / 20.f;
        // printf("%f %f\n", scrollx, scrolly);
    }
}
...
int main(int argc, char** argv) {
    srand(arc4random());
    horizontal = (rand() % 164) -82.f;
    vertical = (rand() % 164) -82.f;
